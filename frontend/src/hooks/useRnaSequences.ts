...
export function useRnaSequences() {
  const [query, setQuery] = useState("");
  const [filters, setFilters] = useState<RnaFilters>(DEFAULT_RNA_FILTERS);
  const [status, setStatus] = useState<RnaStatus>(isApiConfigured ? "idle" : "unavailable");
  const [results, setResults] = useState<RnaSequence[]>([]);
  ...
  useEffect(() => {
    ...
    listRna(debouncedQuery, filters, {
      limit: PAGE_SIZE,
  ...
  const resetFilters = useCallback(() => setFilters(DEFAULT_RNA_FILTERS), []);

  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (filters.organism) count += 1;
    if (filters.source !== "all") count += 1;
    if (filters.rnaClass !== "all") count += 1;
    if (filters.coding !== "all") count += 1;
