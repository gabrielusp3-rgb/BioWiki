...
export function DNAFilters({ filters, onChange, onReset }: DNAFiltersProps) {
  ...
  return (
    <div className="glass hairline grid grid-cols-1 gap-5 p-6 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <Field label="Molecule type">
        <select
          value={filters.moleculeType}
          onChange={(e) =>
            onChange({ ...filters, moleculeType: e.target.value as DnaMoleculeType | "all" })
          }
          className={fieldClass}
        >
          {MOLECULE_TYPE_OPTIONS.map((o) => (
            <option key={o.value} value={o.value} className="bg-bg-secondary">
              {o.label}
            </option>
          ))}
      ...
      <Field label="Strand">
        <select
          ...
          onChange={(e) => onChange({ ...filters, strand: e.target.value as Strand | "all" })}
          className={fieldClass}
        >
          {STRAND_OPTIONS.map((o) => (
            <option key={o.value} value={o.value} className="bg-bg-secondary">
              {o.label}
            </option>
          ))}
